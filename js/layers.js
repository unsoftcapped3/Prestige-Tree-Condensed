addLayer("p", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
          ...
          mult=mult.mul(buyableEffect("condensers",12))
            return mult
        },
		...
		upgrades: {
			...
			22: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e262:250).pow(tmp.h.costExp11) },
				effect() {
					let eff = Decimal.pow(1.4, player.p.upgrades.length);
					if (hasUpgrade("p", 32)) eff = eff.pow(8);
			...
			31: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e316":1e32).pow(tmp.h.costExp11) },
				unlocked() { return hasAchievement("a", 23)&&hasUpgrade("p", 21) },
			},
			32: {
				title: "Still Useless",
				description: "<b>Upgrade Power</b> is raised to the 8th power.",
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e355":1e52).pow(tmp.h.costExp11) },
			...
			33: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e436":1e64).pow(tmp.h.costExp11) },
				effect() { return player.p.total.plus(1).log10().plus(1).log10().div(5).plus(1).times(hasUpgrade("hn", 33) ? upgradeEffect("hn", 33) : 1).pow(1.05).mul(1.05) },
...
addLayer("b", {
        ...
        position: 0, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
        color: "#6e64c4",
        requires() { return new Decimal(1000).times((player.b.unlockOrder&&!player.b.unlocked)?50000:1) }, // Can be a function that takes requirement increases into account
      ...
      2: {
				...
				done() { return player.b.best.gte(2) || hasAchievement("a", 41) || hasAchievement("a", 71) },
				effectDescription: "Keep normal condensers on reset and unlock prestige condensers",
			},
			0: {
			...
			1: {
				...
				effectDescription: "You can buy max Boosters and booster cost base is square rooted.",
			},
		},
		upgrades: {
			rows: 3,
			...
			12: {
				...
				description: "Generators add to the Booster effect base and keep prestige condensers.",
          ...
          ret=ret.add(1).sqr().sub(1)
			...
			22: {
				...
				description: "Raise the Generator Power effect ^1.5.",
			...
			23: {
				...
				effect() { 
					let ret = player.points.add(1).log10().add(1).pow(3.2).pow(2);
...
addLayer("g", {
        ...
        color: "#a3d9a5",
        requires() { return new Decimal(1000).times((player.g.unlockOrder&&!player.g.unlocked)?50000:1) }, // Can be a function that takes requirement increases into account
        resource: "generators", // Name of prestige currency
        baseResource: "points", // Name of resource prestige is based on
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["p"],
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.1:1.2 }, // Prestige currency exponent
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2.5:(hasAchievement("a2",14)?1000:10000) },
		gainMult() {
			let mult = new Decimal(1);
      ...
      eff=eff.mul(buyableEffect("condensers",13))
			return eff;
		},
		effectDescription() {
		...
		milestones: {
			...
			1: {
				...
				effectDescription: "You gain 100% of Prestige Point gain every second, and unlock generator power condensers.",
			},
			2: {
				requirementDescription: "15 Generators",
				done() { return player.g.best.gte(15) || hasAchievement("a", 71) },
				effectDescription: "You can buy max Generators and keep generator power condensers on reset.",
			},
		},
		upgrades: {
			rows: 3,
			...
			11: {
				...
				effect() { return player.g.best.plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?5e5:1.5) },
          ...
          ret=ret.add(1).sqr().sub(1)
			...
			14: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?378:12) },
				unlocked() { return player.g.best.gte(10) },
			},
			15: {
				title: "Outer Synergy",
				description: "<b>Self-Synergy</b> is stronger based on your Generators.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?382:14) },
				effect() { 
			...
			21: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e314":1e14) },
			...
			22: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"5e47141":1e16) },
				currencyDisplayName: "generator power",
                currencyInternalName: "power",
                currencyLayer: "g",
				effect() { 
			...
			23: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"2e47525":1e18) },
				currencyDisplayName: "generator power",
                currencyInternalName: "power",
                currencyLayer: "g",
...
addLayer("h", {
		...
		milestones: {
			0: {
				unlocked() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):hasMilestone("h",0)) },
...
addLayer("m", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
          ...
          mult=mult.mul(tmp.condensers.buyables[32].effect)
...
addLayer("ps", {
      ...
      if(hasAchievement("a2",24))b=b.div("e2200")
		...
		powerGain() { return player.ps.souls.plus(1).times(tmp.ps.buyables[21].effect).times(tmp.n.dustEffs.purple).mul(tmp.condensers.buyables[31].effect) },
		...
		buyables: {
          ...
          if(cost.gte("e4000"))cost=cost.div("e500")
					return cost;
                },
				effect() {
			...
			52: {
				...
				multiRes: [
					...
					},
					{
						...
						currencyInternalName: "points",
						currencyLayer: "p",
						cost() { return new Decimal(player.ma.current=="hn"?"e7.5e11":"e15000000") },
					},
				],
...
addLayer("n", {
        ...
        requires() { return new Decimal((player[this.layer].unlockOrder>0&&!hasAchievement("a", 92))?"1e2771":"1e2571") }, // Can be a function that takes requirement increases into account
		...
		tabFormat() { 
			...
			"blank", "blank", ["row",[["buyable", 11],["buyable",12]]], "blank", "blank",
		]},
		dustGainMult() {
			let mult = new Decimal(1);
			if (player.n.buyables[11].gte(1)) mult = mult.times(buyableEffect("o", 22));
      ...
      if(hasMilestone("hn",9))player.n.unlockOrder=0
		},
		buyables: {
			rows: 1,
			cols: 2,
      ...
      12: {
				title: "More Condensers",
				cap() { return new Decimal(1) },
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					let cost = new Decimal(100).pow(x.add(1).pow(69))
					return cost;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = ((player[this.layer].buyables[this.id].gte(data.cap)?"MAXED":(("Cost: " + formatWhole(data.cost)+" Nebula Energy"+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(data.cap)+"\n\
					Unlocked "+formatWhole(player[this.layer].buyables[this.id])+" new condenser"+(player[this.layer].buyables[this.id].eq(1)?"":"s")))))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
					let cost = tmp[this.layer].buyables[this.id].cost
                    return player.n.unlocked && player.n.points.gte(cost) && player[this.layer].buyables[this.id].lt(tmp[this.layer].buyables[this.id].cap)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
...
addLayer("hs", {
		...
		roundUpCost: true,
        color: "#dfdfff",
        requires() { return new Decimal((player[this.layer].unlockOrder>0&&!hasAchievement("a", 92))?626:302) }, // Can be a function that takes requirement increases into account
      ...
      if(hasMilestone("hn",9))player.hs.unlockOrder=0
		},
		hyperspace() {
		...
		buyables: {
			...
			11: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					...
					let cost = {hs: Decimal.pow(10, y.pow(0.9)).floor(), ba: Decimal.pow(10, x.max(x.div(1.5).pow(2)).times(40).add(player.hs.unlockOrder?1800:360))}
					return cost;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
      ...
      12: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = (player.hs.buyables[12].eq(1)?"MAXED":"Cost: " + formatWhole(data.cost) + " Hyperspace Energy")+"\n\nUnlocked "+formatWhole(player[this.layer].buyables[this.id])+" new condenser"+(player[this.layer].buyables[this.id].eq(1)?"":"s")
                ...
                canAfford() {
					let cost = tmp[this.layer].buyables[this.id].cost
                    return player.hs.unlocked && player.hs.points.gte(cost) && getBuyableAmount(this.layer,this.id).lt(1)
				},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
					player.hs.points = player.hs.points.sub(cost)
                    player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].add(1)
                },
                style() { return {'height':'200px', 'width':'200px'}},
			},
...
addLayer("i", {
        ...
        color: "#e5dab7",
        requires() { return new Decimal("1e78000") }, // Can be a function that takes requirement increases into account
...
addLayer("a", {
        ...
        achievements: {
			...
			23: {
				...
				tooltip: "Reach 1e30 Prestige Points. Reward: Unlock 3 new Prestige Upgrades.",
